...
// alias makes it so we can set up dummy function to hande interrupts 
//		i.e. not implement immediately, let programmer implement
// weak let's programmer override dummy function with same function name in main application
...
// Reset handler is first function to run on startup (after device reset). 
//		So we initialize SRAM, etc here
void Reset_Handler(void)
{
	//copy .data section to SRAM
	uint32_t size = (uint32_t)&_edata - (uint32_t)&_sdata; //because we don't want to access value present 
									   	//at address, we want address to get size
	...
	//call init function of std library (if using things like printf)
	__libc_init_array();
	//call main()
	main();
}